...
def get_configuration(config_path='/app/src/MinMax/config.json'):
    ...
    try:
        with open(config_path, 'r') as file:
            ...
            expected_keys = ["tipos_estudio", "consultas", "hora_inicio", "hora_fin", "intervalo_consultas_minutos", "roles", "personal", "cargos"]
...
def get_aco_params(params_path='aco_params.json'):
    ...
    try:
        ...
        if not isinstance(params["pheromone_max"], (int, float)) or params["pheromone_max"] <= 0:
            raise Exception("'pheromone_max' debe ser un número positivo.")
        if not isinstance(params["pheromone_min"], (int, float)) or params["pheromone_min"] < 0:
            raise Exception("'pheromone_min' debe ser un número no negativo.")
        return params
    except Exception as e:
...
if __name__ == "__main__":
    ...
    # Generar instancias de personal según roles y cantidades
